...
Zrad::Zrad(const std::string name, IGenerator *gen):
  IAlgorithm(gen),
  IMsgService(name)
{
...
StatusCode Zrad::execute( generator::Event &ctx )
{
  ...
  if ( zee.empty() ){
    MSG_DEBUG( "There is not Zrad event inside of this event");
  ...
  MSG_INFO("Filling Zrad events into the context...");
